...
Esta variable representa el saldo anual medio de los clientes.
...
labels = contact$`df_train$education`,
...
Esta variable hace referencia al numero de días después de haber sido contactado el cliente posterior a una campaña
```{r}
pdays <- df_train %>%
  group_by(df_train$education) %>%
  count() %>%
...
pie(x=pdays$n,
    labels = pdays$`df_train$education`,
    radius = 1,
    main = "Número de clientes en función del número de días despues de ser contactados",
    col = viridis(length(pdays$n)))
```

#### Visualización de la variable "previous"
Esta variable hace referencia al numero de veces que fue contactado el cliente
```{r}
...
pie(x=previous$n,
    labels = previous$`df_train$education`,
    radius = 1,
    main = "Número de veces en función de ser contactado el cliente",
    col = viridis(length(previous$n)))
```

#### Visualización de la variable "poutcome"
Esta variable hace referencia al resultado de la campaña anterior
```{r}
poutcome <- df_train %>%
  group_by(df_train$education) %>%
  count() %>%
...
pie(x=poutcome$n,
    labels = poutcome$`df_train$education`,
    radius = 1,
    main = "Número de clientes en función del resultado de la campaña anteior",
    col = viridis(length(poutcome$n)))
```
