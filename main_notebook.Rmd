...
```{r}
dia <- df_train %>%
  mutate(dia_interval = cut_interval(df_train$day, length = 7)) %>%
  group_by(dia_interval) %>%
  count()

# visualizamos eso en un gráfico

 ggplot(dia, aes(x = dia_interval, y=n)) +
   geom_bar(stat="identity") +
   ggtitle("Número de clientes en función del día") +
   labs(x="dia", y = "número de clientes")
```

#### Visualización de la variable "month"
```{r}
mes <- df_train %>%
#No se realiza agrupación
=======
ggplot(df_train, aes(x = duration)) +
  ...
  xlim(0,1000)
>>>>>>> 92346a1007e7613afd9f76a5ed6ee784bc312b8d

# visualizamos eso en un gráfico

 ggplot(mes, aes(x = mes, y=n)) +
   geom_bar(stat="identity") +
   ggtitle("Número de clientes en función del mes") +
   labs(x="mes", y = "número de clientes")
```
...
```
Se ve en el gráfico que hay buenos resultados (precisión superior al 90%) con todos los valores de k, y hay una precisión de más de 92% con k=2 y k=3. Se va a calcular con más detalle la precisión que se obtiene con k = 2.
...
A partir de las visualizaciones, se observó que los datos se agrupan más claramente en dos grupos en comparación con tres o cuatro grupos.
...
De la tabla anterior, se observó que el agrupamiento jerárquico agrupa casi todos los puntos de datos en 1 grupo que es no. K significa que la agrupación en clústeres también agrupa los puntos de datos como sí, por lo que K significa que la agrupación en clústeres funciona mejor que la agrupación jerárquica.

#### Cluseting jerárquico, dendograma

```{r, warning = F}
# realizar agrupaciones jerárquicas

Datos <-read.csv2("data/test.csv")
